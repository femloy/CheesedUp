...
function scr_vigilante_update_sounds()
{
	...
	if state == states.duel or instance_exists(obj_snottycream_cutscene)
...
function scr_vigilante_walk()
{
	...
	&& phase - 1 < array_length(attack_list) && wastedhits < array_length(attack_list[phase - 1])
	{
...
function scr_vigilante_duel()
{
	...
	var _cheat = obj_player1.character == "N" || global.swapmode;
	if duelphase == 2
	{
		with obj_player1
		{
	...
	else if duelphase < 2
	{
		with obj_player1
		{
			if key_slap2 && character != "N" && !_cheat
			{
				other.duelbuffer = 0;
				other.duelphase = 2;
	...
	if duelbuffer > 0
	{
		...
		switch duelphase
		{
			...
			case 1:
				...
				if (obj_player1.character != "N" && signy <= -sprite_get_height(signspr))
				{
					duelbuffer = 80;
		...
		if duelphase == 2 && obj_player1.character != "N" && !_cheat
	...
	else
	{
		switch duelphase
		{
			...
			case 2: // either shoot or get shot
				if !duelplayer
				{
				...
				else
				{
					...
					if snotty
					{
						...
						with obj_player1
							state = states.actor;
						instance_create_unique(0, 0, obj_snottycream_cutscene);
					}
					else
					{
						sound_play("event:/sfx/vigilante/finalshot");
			...
			case 3: // return to battle after shooting player
				if floor(image_index) == image_number - 1
				{
					sprite_index = spr_playerV_idle;
					state = states.walk;
