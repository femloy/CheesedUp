...
{
	if live_call(x, y, xscale, yscale, tex_x, tex_y, sprite, bubble_sprite, txo, tyo) return live_result;
	
	var w = sprite_get_width(bubble_sprite) * xscale;
	var h = sprite_get_height(bubble_sprite) * yscale;
	
	if !variable_instance_exists(id, "surffinal")
	{
		surfclip = noone;
		surffinal = noone;
	}
		...
		switch t
		{
			case texttype.icon:
				...
				switch val
				{
					...
					// pto
                    case textkey.chainsaw:
                        icon = tdp_get_tutorial_icon("player_chainsaw")
                        break
				}
