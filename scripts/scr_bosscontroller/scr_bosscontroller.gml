...
{
	if boss_func != noone
		boss_func(); // boss_hp is set by this
	...
	if boss_prevhp != boss_hp
	{
		if boss_prevhp > boss_hp
		{
			...
			if !is_undefined(pos) && is_array(pos)
			{
				...
				{
					var peppalette = get_pep_palette_info();
					
					bpal = spr_peppalette;
					bpalsel = peppalette.paletteselect;
					btex = peppalette.patterntexture;
				}
	...
	if (!instance_exists(bossID) && !bossdead && instance_exists(obj_player1) && obj_player1.state != states.tackle && obj_player1.state != states.comingoutdoor && room != boss_fakepephallway && room != boss_pizzaface && room != rank_room)
	{
		if (room != boss_vigilante || obj_player1.state != states.actor)
		{
			...
			with (obj_player1)
			{
				...
				fmod_event_instance_set_parameter(global.snd_bossbeaten, "state", st, true);
...
{
	if live_call(sprite, rows, columns, hp, maxhp, x, y, xpad, ypad, index, alpha, spr_palette, paletteselect, palettetexture) return live_result;
