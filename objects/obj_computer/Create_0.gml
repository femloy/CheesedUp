...
update_text = function()
{
	...
	else if text != 2
	{
		if global.sandbox
		{
			var str = lang_get_value_newline("computer_text2_sandbox");
			str = embed_value_string(str, [achievements, maxachievements, damage]);
		}
		else
		{
			var str = lang_get_value_newline("computer_text2");
			str = embed_value_string(str, [achievements, maxachievements, palettes, maxpalettes, damage]);
		}
	}
	else
	{
		var str = lang_get_value_newline("computer_text3");
		str = embed_value_string(str, [laps, retries]);
	}
