...
if sandbox
{
	...
	if sprite_index == selectedspr
	{
		...
		draw_sprite_ext(spr_sandboxtvs, noisehead, xx + noisexoffset + random_range(-1, 1), yy + noiseyoffset + random_range(-1, 1), noisescale + sin(timer / 50) * .5, noisescale + sin(timer / 60 + pi) * .5, sin(timer / 200 + pi / 2) * 25, c_white, 1);
	}
	if sprite_index == confirmspr
	{
		...
		draw_sprite_ext(spr_sandboxtvs, noisehead, xx + noisexoffset + random_range(-1, 1), yy + noiseyoffset + random_range(-1, 1), scale, scale, 0, c_white, 1);
	}
