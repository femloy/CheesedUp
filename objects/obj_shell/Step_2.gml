...
if (!isOpen) {
	...
	// selection
	var do_selection = keyboard_check(vk_shift);
	if do_selection && (keyboard_check(vk_right) - keyboard_check(vk_left)) != 0 && selection == noone
		selection = [cursorPos, cursorPos];
	else if selection != noone
	{
		if do_selection
		...
		var cut = _key_combo_pressed([metaKey], ord("X"));
		
		if keyboard_check_pressed(vk_backspace) or keyboard_check_pressed(vk_delete)
		or keyboard_string != "" or _key_combo_pressed([metaKey], ord("V")) or cut
		{
			var leftmost = min(selection[0], selection[1]);
			var count = max(selection[0], selection[1]) - min(selection[0], selection[1]);
			
			if cut
				clipboard_set_text(string_copy(consoleString, leftmost, count));
			
			consoleString = string_delete(consoleString, leftmost, count);
			cursorPos = leftmost;
			selection = noone;
			...
			if _keyboard_check_delay(vk_backspace)
				exit;
			if _keyboard_check_delay(vk_delete)
				exit;
		}
	}
	
	// copy pasting
	if keyboard_check(metaKey)
	{
		if keyboard_check_pressed(ord("V")) && clipboard_has_text()
		{
			consoleString = string_insert(clipboard_get_text(), consoleString, cursorPos);
	...
	if (keyboard_check_pressed(vk_escape)) {
		...
		if !do_selection && selection != noone
		{
			cursorPos = min(selection[0], selection[1]);
			selection = noone;
		}
		else if (!metaMovedLeft) {
			cursorPos = max(1, cursorPos - 1);
			targetScrollPosition = maxScrollPosition;
		}
		...
		if !do_selection && selection != noone
		{
			cursorPos = max(selection[0], selection[1]);
			selection = noone;	
		}
		else if (!metaMovedRight) {
